...
class MailThread(models.AbstractModel):
    ...
    def _search_follower_channels(self, operator, operand):
        """Search function for message_channel_ids"""
        # ideal solution, but crashes for security reasons.
        # Generic Users have no right to access message_followers records
        # Furthermore, currently the ORM adds a useless subquery, like the second solution
        # return [('message_follower_ids.channel_id', operator, operand)]

        # Second solution, works but one query depth for nothing due to the id in
        # (And one more useless depth because of the current ORM implementation of x2m in expression.py)
        # return [('id', 'in', self.sudo()._search([('message_follower_ids.channel_id', operator, operand)], order="id"))]
