...
class MessageList extends Component {
    ...
    async _adjustFromMessageReceived(hint) {
        ...
        if (!threadCache.__mfield_messages(this).includes(message)) {
            return;
        }
        if (!this.messageRefFromId(message.__mfield_id(this))) {
            return;
        }
        if (!this.props.hasScrollAdjust) {
            this.threadView.markComponentHintProcessed(hint);
        ...
        if (!this.threadView.__mfield_hasAutoScrollOnMessageReceived(this)) {
            this.threadView.markComponentHintProcessed(hint);
            return;
        }
        if (
            this.threadView.__mfield_lastVisibleMessage(this) &&
            (message.__mfield_id(this) < this.threadView.__mfield_lastVisibleMessage(this).__mfield_id(this))
        ) {
            this.threadView.markComponentHintProcessed(hint);
            return;
        }
        await this._scrollToMessage(message.__mfield_id(this));
        this.threadView.markComponentHintProcessed(hint);
    }
    ...
    _onScroll(ev) {
        ...
        this.threadView.update({
            __mfield_hasAutoScrollOnMessageReceived: hasAutoScrollOnMessageReceived,
        });
