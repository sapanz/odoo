...
QUnit.test("Display suggested recipients list", async function (assert) {
    ...
    assert.containsOnce(
        ...
        "Must containt a suggested recipient list"
    );
    assert.containsN(
        document.body,
        '.o_ComposerSuggestedRecipient',
        3,
        "at start, the list should not contain more than 3 suggested recipients"
    );
    const title = document.querySelector('.o_ComposerSuggestedRecipientList .o_ComposerSuggestedRecipient:first-child').getAttribute('title');
    assert.strictEqual(
        title,
        "Add as recipient and follower (reason: CC email)",
        "reason must be present in the suggested recipient title",
    );
    const checkboxUnchecked = document.querySelector('.o_ComposerSuggestedRecipientList .o_ComposerSuggestedRecipient:first-child input[type=checkbox]');
    assert.notOk(
        checkboxUnchecked.checked,
        "suggested recipient without a partner should not be checked by default",
    );
    const checkboxChecked = document.querySelector('.o_ComposerSuggestedRecipientList .o_ComposerSuggestedRecipient input.o-has-partner');
    assert.ok(
