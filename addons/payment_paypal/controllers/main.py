...
class PaypalController(http.Controller):
    ...
    def _paypal_validate_data(self, **post):
        """ Paypal IPN: three steps validation to ensure data correctness
         ...
         - step 2: POST the complete, unaltered message back to Paypal (preceded
           by cmd=_notify-validate or _notify-sync for PDT), with same encoding
        ...
        reference_key = 'item_number' + post.get('num_cart_items', '')
        reference = post.get(reference_key)
        tx = None
        if reference:
        ...
        if not tx:
            ...
            return False
        paypal_url = tx.acquirer_id._get_redirect_action_url()
        ...
        if resp in ['VERIFIED', 'SUCCESS']:
            ...
            res = tx.sudo()._process_feedback_data(post)
            if not res and tx:
            ...
            if tx:
                tx._set_error('Invalid response from Paypal. Please contact your administrator.')
        else:
            ...
            if tx:
                tx._set_error('Unrecognized error from Paypal. Please contact your administrator.')
        return res

    @http.route('/payment/paypal/ipn/', type='http', auth='public', methods=['POST'], csrf=False)
    def paypal_ipn(self, **post):
        """ Paypal IPN. """
        # TODO replace XXXX, where am I supposed to find card masked number ?
        _logger.info('Beginning Paypal IPN XXXX with post data %s', pprint.pformat(post))  # debug
        try:
            self._paypal_validate_data(**post)
    ...
    def paypal_dpn(self, **post):
        """ Paypal DPN """
        # TODO replace XXXX, where am I supposed to find card masked number ?
        _logger.info('Beginning Paypal DPN XXXX with post data %s', pprint.pformat(post))  # debug
        try:
            self._paypal_validate_data(**post)
        except ValidationError:
            _logger.exception('Unable to validate the Paypal payment')
