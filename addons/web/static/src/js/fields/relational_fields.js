...
var FieldReference = FieldMany2One.extend({
    ...
    start: function () {
        ...
        this.$('select').val(this.modelName);
        return this._super.apply(this, arguments);
    },
    ...
    /**
     * Apply the model contained in the option model_field. and re-initialize the record if the model change.
     * @param {Boolean} initRecord :true, re-initialize the record if the model change. Necessary for wizards.
     */
    _applyModelField: function (initRecord) {
        let resourceRef = this.record.specialData[this.name];
        if (resourceRef) {
            if (initRecord && resourceRef.isModified) {
                this.reinitialize(false);
            }
    ...
    _onSelectionChange: function () {
        this.modelName = this.$('select').val();
        this.reinitialize(false);
        this._setRelation(this.modelName);
    },
});
