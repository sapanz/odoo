...
publicWidget.registry.SurveySessionManage = publicWidget.Widget.extend({
    ...
    _onBack: function (ev) {
        ...
        } else if (this.currentScreen === 'question') {  // If go back to previous question
    ...
    */
    ...
    _getPreviousScreen: function () {
        if (this.currentScreen === 'userInputs' && this.isScoredQuestion) {
            ...
            return 'userInputs';
        } else if (this.currentScreen === "question" || this.currentScreen === 'leaderboard' && this.isScoredQuestion) {
            return 'results';
        }
        ...
        Promise.all([fadeOutPromise, nextQuestionPromise]).then(function (results) {
            if (results[1]) {
                ...
                self.$el.fadeIn(self.fadeInOutTime, function () {
                    if (goBack) {
                        // As we arrive on first "question" screen, simulate going to the results.
