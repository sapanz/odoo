...
class Meeting(models.Model):
    ...
    def unlink(self):
        ...
        actual_partner = self.env.user.partner_id
        events_to_remove = self.env['calendar.event']
        for event in self:
            if actual_partner.id in event.partner_ids.ids and actual_partner.id != event.partner_id.id:
                event.attendee_ids.filtered(lambda attendee: attendee.partner_id == actual_partner).state = 'declined'
            elif actual_partner.id == event.partner_id.id or event.privacy == 'public':
                events_to_remove += event
        result = super(Meeting, events_to_remove).unlink()
