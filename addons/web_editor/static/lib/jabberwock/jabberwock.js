...
(function (exports, owl) {
            ...
            for (let unitIndex = 0; unitIndex < renderingUnits.length; unitIndex++) {
                ...
                if (unit && unit[1].length) {
                    ...
                    const modifierPromise = Promise.all(promises).then(async () => {
                        ...
                        for (const node of nodes) {
                            const wrap = wraps.find(wrap => cache.renderingDependent.get(wrap)?.has(node));
    ...
    class DefaultHtmlTextParser extends AbstractParser {
        async parse(item) {
            ...
            const template = item
                .replace(autoCloseRegExp, match => match[match.length - 2] === '/' ? match : match.slice(0, -1) + '/>')
                .replace(/&/g, '&amp;');
                ...
                const handlerMouseDown = (ev) => {
                    if (ev.offsetX < 0) {
                        ...
                        this.engine.editor.execCommand(() => {
                            return this.engine.editor.execWithRange(VRange.at(listItem.firstChild() || listItem), 'toggleChecked');
                        });
                    }
                };
                li.attach = (el) => {
                        ...
                        for (const index in subValues) {
                            ...
                            if (!isNaN(numeric) && subValue.includes('em')) {
                                ...
                                if (!subValue.includes('rem') && inherit['font-size']) {
                                    ...
                                    previous = this._getDefaultFontSize(cache, ancestors[0]?.domObject.tag || 'p');
                                }
                                subValues[index] =
                                    numeric * previous +
                                        'px' +
    ...
    class ImageMailObjectRenderer extends NodeRenderer {
        ...
        async render(img, worker) {
            ...
            if (imgObject.attributes.class.has('mx-auto') &&
    ...
    class TableCellMailObjectRenderer extends NodeRenderer {
        ...
        async render(cell, worker) {
            ...
            const textAlign = cellObject.attributes.style?.['text-align'] || styleFromRules.current['text-align'];
    ...
    class TablePickerCellDomObjectRenderer extends NodeRenderer {
        constructor() {
            super(...arguments);
            this.predicate = (node) => node instanceof TableCellNode && node.parent?.parent instanceof TablePickerNode;
        }
    ...
    class DomReconciliationEngine {
        ...
        _diffObject(nodeToDomObject, domObject, fromNodes, mapOldIds, childrenMapping) {
            ...
            if (!domNodes.length && domObject.tag) {
                ...
                const oldAttributes = old?.object.attributes || {};
    ...
    class Toolbar extends JWPlugin {
        constructor() {
            ...
            this.configuration = { layout: [], ...this.configuration };
        }
    ...
    class FullscreenButtonDomObjectRenderer extends NodeRenderer {
        ...
        async render(button, worker) {
            ...
            domObject.handler = () => {
                ...
                const component = domLayoutEngine.components[fullscreenPlugin.configuration.component]?.[0];
                if (component) {
        ...
        async render(node, worker) {
            ...
            `;
                ...
                for (const link of links) {
                    link.addEventListener('click', async () => this.engine.editor.execCommand(link.dataset.commandId));
                }
                let positionable;
                let commitHandler;
                table.attach = (el) => {
    ...
    class FontAwesomeMailObjectRenderer extends NodeRenderer {
        ...
        async render(node, worker) {
            ...
            const weight = parseInt(styleFromRules.current['font-weight'] || styleFromRules.inherit['font-weight'] || '400', 10);
         ...
         * @param weight
         */
        _fontToBase64(fontFamily, font, fontSize = 14, color = '#000000', weight = 400) {
    ...
    class IframeHtmlDomParser extends AbstractParser {
        ...
        async parse(item) {
            ...
            const childNodes = item.src
                ? []
                : Array.from(item.contentWindow?.document.body?.childNodes || []);
    ...
    `;
         ...
         */
        async getValue(format = HtmlDomRenderingEngine.id) {
            ...
            const result = await renderer.render(format, editable);
        ...
        async getTextValue() {
            ...
            const domLayout = layout.engines.dom;
            const editable = domLayout.components.editable[0];
            const nodes = await renderer.render(HtmlDomRenderingEngine.id, editable);
            return nodes && nodes.map(n => n.outerHTML).join('');
        }
    }
