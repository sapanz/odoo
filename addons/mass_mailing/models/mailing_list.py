...
class MassMailingList(models.Model):
    ...
    def action_view_contacts_opt_out(self):
        ...
        action['context'] = {'default_list_ids': self.ids, 'create': False}
        return action
    ...
    def _fetch_contact_nbr_counts(self):
        ...
        (see '_get_contact_nbr_select_fields' for details)
        ...
        if self.ids:
            self.env.cr.execute(f'''
                SELECT
                    ...
                    {','.join(self._get_contact_nbr_select_fields().values())}
                FROM
        ...
        for mass_mailing in self:
            ...
            if mass_mailing.id not in contact_nbr_counts:
                contact_nbr_counts[mass_mailing.id] = {
                    field: 0
                    for field in self._get_contact_nbr_select_fields().keys()
                }
    ...
    def _get_contact_nbr_select_fields(self):
        ...
        return {
            'contact_nbr': 'COUNT(*) AS contact_nbr',
            'contact_nbr_email': '''
                SUM(CASE WHEN
            ...
            'contact_nbr_blacklisted': f'''SUM(CASE WHEN {self._get_blacklisted_condition()}
