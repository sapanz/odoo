...
class AccountMove(models.Model):
    ...
    def _populate_factories(self):
        ...
        def search_journal_ids(company_id, journal_type):
            ...
            """
            return self.env['account.journal'].search([('company_id', '=', company_id), ('type', '=', journal_type)]).ids
        ...
        def search_partner_ids(company_id):
            ...
            """
            return self.env['res.partner'].search(['|', ('company_id', '=', company_id), ('company_id', '=', False),
                                                   ('id', 'in', self.env.registry.populated_models['res.partner'])]).ids
        ...
        def get_invoice_date(values, **kwargs):
            ...
            :return (datetime.date, bool): the accounting date if it is an invoice (or similar) document
                                           or False otherwise.
            """
            return values['date'] if values['move_type'] in self.get_invoice_types(include_receipts=True) else False
        ...
        def get_lines(random, values, **kwargs):
            ...
            def get_line(account, label, balance=None, balance_sign=False, exclude_from_invoice_tab=False):
                is_foreign_currency = currency_id != account.currency_id.id and currency_id != account.company_id.currency_id.id
        ...
        return [
            ('move_type', populate.randomize(['entry', 'in_invoice', 'out_invoice', 'in_refund', 'out_refund', 'in_receipt', 'out_receipt'], [0.2, 0.3, 0.3, 0.07, 0.07, 0.03, 0.03])),
