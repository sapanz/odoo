...
class AccountMove(models.Model):
    ...
    date = fields.Date(
        string='Date',
        store=True, readonly=False, index=True, copy=False, # required=True but not supported as it's a computed field.
    ...
    line_ids = fields.One2many(
        ...
        string='Journal Items', copy=True)
    ...
    is_move_sent = fields.Boolean(
        readonly=True, default=False, copy=False, tracking=True,
    ...
    def _recompute_tax_lines(self, recompute_tax_base_amount=False):
        ...
        for line in self.line_ids.filtered(lambda line: not line.tax_repartition_line_id):
            ...
            if not line.tax_ids:
                to_write_line['tax_tag_ids'] = [(5, 0, 0)]
                continue
        ...
        for taxes_map_entry in taxes_map.values():
            ...
            if taxes_map_entry['tax_line'] and not taxes_map_entry['grouping_dict']:
                to_write['line_ids'].append((2, taxes_map_entry['tax_line'].id))
                continue
            ...
            if taxes_map_entry['tax_line'] and recompute_tax_base_amount:
                to_write['line_ids'].append((1, taxes_map_entry['tax_line'].id, {'tax_base_amount': tax_base_amount}))
                continue

            if taxes_map_entry['tax_line']:
                # Update an existing tax line.
                to_write['line_ids'].append((1, taxes_map_entry['tax_line'].id, to_write_line))
            else:
    ...
    def update_lines_tax_exigibility(self):
        if all(account.user_type_id.type not in {'payable', 'receivable'} for account in self.line_ids.account_id):
            ...
            base_lines_caba = self.line_ids.filtered(lambda x: any(tax.tax_exigibility == 'on_payment'
                                                                   for tax in x.tax_ids.flatten_taxes_hierarchy()))
    ...
    def _recompute_cash_rounding_lines(self):
        ...
        def _compute_cash_rounding(self, total_amount_currency):
            ...
            diff_balance = self.currency_id._convert(diff_amount_currency, self.company_id.currency_id, self.company_id, self.date or fields.Date.context_today(self))
        ...
        def _apply_cash_rounding(self, diff_balance, diff_amount_currency, cash_rounding_line):
            ...
            if cash_rounding_line:
                ...
                to_write['line_ids'].append((0, 0, rounding_line_vals))

        existing_cash_rounding_line = self.line_ids.filtered(lambda line: line.is_rounding_line)

        if not self.invoice_cash_rounding_id and not existing_cash_rounding_line:
            return

        not_term_lines = self.line_ids.filtered(lambda line: line.account_id.user_type_id.type not in ('receivable', 'payable'))
        other_lines = not_term_lines - existing_cash_rounding_line

        # The cash rounding has been removed.
        if not self.invoice_cash_rounding_id:
            to_write['line_ids'].append((2, existing_cash_rounding_line.id))
    ...
    def _recompute_payment_terms_lines(self):
        ...
        def _compute_diff_payment_terms_lines(self, existing_terms_lines, account, to_compute):
            ...
            for date_maturity, balance, amount_currency in to_compute:
                ...
                to_write['invoice_date_due'] = date_maturity

            for i in range(existing_terms_lines_index, len(existing_terms_lines)):
                to_write['line_ids'].append((2, existing_terms_lines[i].id))
    ...
    def _recompute_partner_has_changed(self):
        ...
        payable_account = self.partner_id.commercial_partner_id.property_account_payable_id

        if self.is_sale_document(include_receipts=True):
            if not receivable_account:
                return
            new_account = receivable_account
        elif self.is_purchase_document(include_receipts=True):
            if not payable_account:
                return
            new_account = payable_account
        else:
            return
    ...
    def _recompute_dynamic_lines(self, snapshot0):
        ...
        '''
        # Recompute currency rate.
        currency_has_changed = snapshot0.has_changed('currency_id')
        date_has_changed = snapshot0.has_changed('date')
        if (self.is_invoice(include_receipts=True) and currency_has_changed) or date_has_changed:
        ...
        if self.is_invoice(include_receipts=True):
            ...
            if snapshot0.has_changed('payment_reference'):
                self._recompute_payment_reference_has_changed()

            if snapshot0.has_changed('partner_id'):
                self._recompute_partner_has_changed()
    ...
    def _onchange_partner_id(self):
        ...
        warning = {}
        rec_account = self.partner_id.commercial_partner_id.property_account_receivable_id
        pay_account = self.partner_id.commercial_partner_id.property_account_payable_id
    ...
    def _compute_journal_id(self):
        for move in self:
            ...
            if self._context.get('default_journal_id'):
                move.journal_id = self.env['account.journal'].browse(self._context['default_journal_id'])

                if move.move_type != 'entry' and move.journal_id.type not in journal_types:
                    raise UserError(_(
                ...
                move.journal_id = self\
                    .with_context(default_company_id=company.id, default_currency_id=move.currency_id.id)\
    ...
    def _compute_currency_id(self):
        for move in self:
            move.currency_id = move.journal_id.currency_id \
    ...
    def _compute_invoice_date_due(self):
        for move in self:
            if move.invoice_payment_term_id:
                move.invoice_date_due = move.invoice_date_due
            elif move.invoice_date:
                move.invoice_date_due = move.invoice_date
            elif move.is_invoice(include_receipts=True):
                move.invoice_date_due = fields.Date.context_today(move)
            else:
                move.invoice_date_due = move.invoice_date_due
    ...
    def _compute_fiscal_position_id(self):
        for move in self:
            move.fiscal_position_id = self.env['account.fiscal.position'].get_fiscal_position(
                move.partner_id.id,
                delivery_id=move._get_invoice_delivery_partner_id(),
            )

    @api.depends('move_type', 'partner_id', 'journal_id')
    def _compute_partner_bank_id(self):
        for move in self:
            if move.is_outbound():
                customer = move.partner_id
                if move.partner_bank_id.partner_id == customer:
                    move.partner_bank_id = move.partner_bank_id
                else:
                    move.partner_bank_id = customer.bank_ids[0] if customer.bank_ids else False
            elif move.is_inbound():
                company_partner = move.journal_id.company_id.partner_id
                if move.partner_bank_id.partner_id == company_partner:
                    move.partner_bank_id = move.partner_bank_id
                else:
                    move.partner_bank_id = company_partner.bank_ids[0] if company_partner.bank_ids else False
    ...
    @api.depends('name', 'highest_name')
    def _compute_show_name_warning(self):
        for move in self:
            if move.name and move.name != '/' and move.name <= (move.highest_name or ''):
                move.show_name_warning = True
            else:
                move.show_name_warning = False
    ...
    def _compute_bank_partner_id(self):
        for move in self:
            if move.is_outbound():
                move.bank_partner_id = move.partner_id.commercial_partner_id
            else:
    ...
    def _synchronize_business_models(self):
        ...
        self_sudo = self.sudo().with_context(write_recursion=True)
    ...
    def _check_required_date(self):
        for move in self:
            if not move.date:
                raise ValidationError(_("'%(field)s' is a required field", field=move._fields['date'].string))
    ...
    def _check_duplicate_supplier_reference(self):
        ...
        self.env["account.move"].flush([
            "ref", "move_type", "invoice_date", #"journal_id" TODO: RCO FIX ME
    ...
    def _check_balanced(self):
        ...
        '''
        moves = self.filtered(lambda move: move.id and move.line_ids)
        if not moves:
            return
    ...
    def _check_fiscalyear_lock_date(self, date=None):
        is_manager = self.user_has_groups('account.group_account_manager')
        for move in self:
    ...
    def create(self, vals_list):
        ...
        for vals in vals_list:
            ...
            # Ensure the 'post' will always be called on the journal entry.
            ...
            write_vals_list.append(write_vals)
        ...
        moves.update_lines_tax_exigibility()

        if self._context.get('check_move_validity', True):
            moves._check_balanced()

        # Cleanup context.
        return self.browse(moves.ids)
    ...
    def write(self, vals):
        ...
        if not self._context.get('write_recursion'):
            for move, vals, snapshot0 in zip(self.with_context(write_recursion=True), vals_list, snapshots0):
                ...
                # Check restrict mode.
                if move.restrict_mode_hash_table and move.state == 'posted' and snapshot0.any_has_changed(INTEGRITY_HASH_MOVE_FIELDS):
                    raise UserError(_(
                ...
                # Check sequence.
                if move.journal_id.sequence_override_regex and snapshot0.has_changed('name') and not re.match(move.journal_id.sequence_override_regex, move.name):
                    if not move.env.user.has_group('account.group_account_manager'):
                        raise UserError(_("The Journal Entry sequence is not conform to the current format. "
                                          "Only the Advisor can change it."))
                    move.journal_id.sequence_override_regex = False

                # Check writing journal on posted move.
                if move.posted_before and snapshot0.has_changed('journal_id'):
                ...
                if snapshot0.any_has_changed(['date', 'state']):
                    move._check_fiscalyear_lock_date(date=snapshot0['date'])
                    move._check_fiscalyear_lock_date()
                    if snapshot0['state'] == 'posted':
                        move.line_ids._check_tax_lock_date(date=snapshot0['date'])
                    if move.state == 'posted':
                        move.line_ids._check_tax_lock_date()

                # Apply the restrict mode.
                if snapshot0.any_has_changed(['state']) \
    ...
    def _reverse_moves(self, default_values_list=None, cancel=False):
        ...
        for move, reverse_move in zip(self, reverse_moves.with_context()):
            # Update amount_currency if the date has changed.
            if move.date != reverse_move.date:
                reverse_move._recompute_rate_changed()
    ...
    def action_switch_invoice_into_refund_credit_note(self):
        ...
        lines_to_write_list = []
        for move in self:
            reverse_move_vals = move._reverse_move_vals({}, False)

            # Process lines.
            lines_to_write = []
            for cmd, virtualid, line_vals in reverse_move_vals['line_ids']:
                ...
                lines_to_write.append((0, 0, line_vals))
            lines_to_write_list.append(lines_to_write)

        # /!\ Don't recompute anything based on the move_type.
        self._cr.execute('''
            UPDATE account_move
            SET move_type = REPLACE(move_type, 'invoice', 'refund')
            WHERE id IN %s
        ''', [tuple(self.ids)])
        self.invalidate_cache(fnames=['move_type'], ids=self.ids)

        for move, lines_to_write in zip(self, lines_to_write_list):
            move.write({'invoice_line_ids': [(5, 0, 0)]})
            move.write({'invoice_line_ids': lines_to_write})
...
class AccountMoveLine(models.Model):
    ...
    currency_id = fields.Many2one(
        comodel_name='res.currency',
        string='Currency',
        store=True, readonly=False, # required=True but not supported as it's a computed field.
        compute='_compute_currency_id')
    ...
    def _get_computed_taxes(self):
        ...
        if self.move_id.is_sale_document(include_receipts=True):
            ...
            if self.product_id.taxes_id:
                taxes = self.product_id.taxes_id.filtered(lambda tax: tax.company_id == self.move_id.company_id)
            elif self.account_id.tax_ids:
                taxes = self.account_id.tax_ids
            else:
                taxes = self.env['account.tax']
            if not taxes and not self.exclude_from_invoice_tab:
                taxes = self.move_id.company_id.account_sale_tax_id
        elif self.move_id.is_purchase_document(include_receipts=True):
            # In invoice.
            if self.product_id.supplier_taxes_id:
                taxes = self.product_id.supplier_taxes_id.filtered(lambda tax: tax.company_id == self.move_id.company_id)
            elif self.account_id.tax_ids:
                taxes = self.account_id.tax_ids
            else:
                taxes = self.env['account.tax']
            if not taxes and not self.exclude_from_invoice_tab:
                taxes = self.move_id.company_id.account_purchase_tax_id
        else:
            # Miscellaneous operation.
            taxes = self.account_id.tax_ids

        if self.company_id and taxes:
    ...
    def _update_accounting_fields_from_price_subtotal(self, force_override=False):
        self.ensure_one()
        ...
        if move.is_outbound(include_receipts=True):
            sign = 1
        elif move.is_inbound(include_receipts=True):
            sign = -1
        else:
            sign = 1
        ...
        # Update accounting fields if necessary.
        new_amount_currency = self.price_subtotal * sign
        if force_override or not currency.is_zero(self.amount_currency - new_amount_currency):
    ...
    def _update_business_fields_from_price_subtotal(self, price_subtotal):
        self.ensure_one()

        company_currency = self.move_id.journal_id.company_id.currency_id
        currency = self.currency_id or company_currency
        price_subtotal = currency.round(price_subtotal)

        # Update business fields if necessary.
        if currency.is_zero(self.price_subtotal - price_subtotal):
            return

        discount_factor = 1 - (self.discount / 100.0)
        price_subtotal_to_inverse = price_subtotal
        to_write = {}
        ...
        if self.tax_ids:
            ...
            if taxes and any(tax.price_include for tax in taxes):
                ...
                # 20            |               | 10% incl          | 20                | 20
                # 10            |               | 5%                | 10                | 10
                taxes_res = taxes._origin.compute_all(price_subtotal_to_inverse, currency=currency, handle_price_include=False)
                for tax_res in taxes_res['taxes']:
                    tax = self.env['account.tax'].browse(tax_res['id'])
                    if tax.price_include:
                        price_subtotal_to_inverse += tax_res['amount']

        if not currency.is_zero(price_subtotal_to_inverse) and discount_factor:
            # discount != 100%
            to_write.update({
                'quantity': self.quantity or 1.0,
                'price_unit': price_subtotal_to_inverse / discount_factor / (self.quantity or 1.0),
            })
        elif not currency.is_zero(price_subtotal_to_inverse) and not discount_factor:
            # discount == 100%
            to_write.update({
                'quantity': self.quantity or 1.0,
                'discount': 0.0,
                'price_unit': price_subtotal_to_inverse / (self.quantity or 1.0),
            })
        elif currency.is_zero(price_subtotal_to_inverse) and discount_factor:
    ...
    def _onchange_product_id(self):
        if self.product_id and self.display_type not in ('line_section', 'line_note'):
    ...
    def _onchange_balance(self):
        if self.currency_id == self.company_currency_id:
            self.amount_currency = self.balance
        else:
            return

        if not self.move_id.is_invoice(include_receipts=True) \
                or self.exclude_from_invoice_tab \
                or self.display_type in ('line_section', 'line_note'):
            return

        if self.move_id.is_outbound(include_receipts=True):
            self._update_business_fields_from_price_subtotal(self.balance)
        elif self.move_id.is_inbound(include_receipts=True):
            self._update_business_fields_from_price_subtotal(-self.balance)
    ...
    def _onchange_amount_currency(self):
        ...
        if self.move_id.is_invoice(include_receipts=True) and not self.exclude_from_invoice_tab:
            ...
            price_subtotal = self.amount_currency * sign
            self._update_business_fields_from_price_subtotal(price_subtotal)

        # Update 'balance' from 'amount_currency'.
        balance = self.currency_id._convert(
    ...
    def _onchange_price_subtotal(self):
        if self.move_id.is_invoice(include_receipts=True) and not self.exclude_from_invoice_tab:
            self._update_accounting_fields_from_price_subtotal()
    ...
    def _onchange_currency(self):
        for line in self:
            ...
            if move.is_invoice(include_receipts=True) and not line.exclude_from_invoice_tab:
                ...
                # Recompute 'debit' / 'credit' / 'amount_currency' from 'price_subtotal'.
                line._update_accounting_fields_from_price_subtotal(force_override=True)
    ...
    @api.depends('move_id.move_type', 'move_id.currency_id')
    def _compute_currency_id(self):
        for line in self:
            if line.move_id.is_invoice(include_receipts=True):
                line.currency_id = line.move_id.currency_id or line.move_id.company_currency_id
            else:
    ...
    def _compute_price_unit(self):
        for line in self:
            ...
            # Compute price_unit from product.
            if line.product_id:
                ...
                if move.is_sale_document(include_receipts=True):
                    ...
                    product_taxes = line.product_id.taxes_id.filtered(lambda tax: tax.company_id == line.company_id)
                elif move.is_purchase_document(include_receipts=True):
                    # In invoice.
                    price_unit = line.product_id.standard_price
                    product_taxes = line.product_id.supplier_taxes_id.filtered(lambda tax: tax.company_id == line.company_id)
                else:
                    price_unit = line.price_unit
                    product_taxes = self.env['account.tax']

                # Manage the fiscal position.
                ...
                if move.fiscal_position_id:
                    ...
                    if any(tax.price_include for tax in product_taxes):
                        taxes_res = product_taxes.compute_all(
                            price_unit,
                    ...
                    mapped_product_taxes = move.fiscal_position_id.map_tax(product_taxes)
                    if any(tax.price_include for tax in mapped_product_taxes):
                        taxes_res = mapped_product_taxes.compute_all(
                            price_unit,
                            product=line.product_id,
                            partner=line.partner_id,
                            is_refund=move.move_type in ('out_refund', 'in_refund'),
                ...
                # Manage the uom.
                # If another unit of measure is set on the line, the price_unit should be adapter accordingly.
                ...
                # The price_unit set on the product is expressed on its related company.
    ...
    def _compute_name(self):
        for line in self:
            ...
            if line.product_id:
                ...
                if move.is_sale_document(include_receipts=True) and product.description_sale:
                    values.append(product.description_sale)
                elif move.is_purchase_document(include_receipts=True) and product.description_purchase:
    ...
    def _compute_price_subtotal(self):
        for line in self:
            move = line.move_id
            currency = line.currency_id or move.journal_id.company_id.currency_id
            ...
            if line.tax_ids:
                ...
                line.price_subtotal = currency.round(taxes_res['total_excluded'])
                line.price_total = currency.round(taxes_res['total_included'])
            else:
                line.price_subtotal = currency.round(line.quantity * price_unit_wo_discount)
    ...
    def _compute_analytic_account(self):
        for record in self:
            if not record.exclude_from_invoice_tab:
                ...
                record.analytic_account_id = rec.analytic_id if rec else record.analytic_account_id
                record.analytic_tag_ids = rec.analytic_tag_ids if rec else record.analytic_tag_ids
            else:
    ...
    def _check_accountable_required_fields(self):
        for line in self:
            if line.display_type:
                if line.amount_currency or line.balance or line.account_id:
                    raise ValidationError("Forbidden unit price, account and quantity on non-accountable invoice line.")
    ...
    def _check_required_currency(self):
        ...
        for line in self:
            if not line.currency_id:
                raise ValidationError(_("The 'currency_id' field is mandatory on account.move.line."))
    ...
    def _check_amount_currency_equals_balance(self):
        for line in self:
            if line.currency_id == line.company_currency_id and not line.currency_id.is_zero(line.amount_currency - line.balance):
                raise ValidationError(_(
                    "If the currency is the same as the one from the company, this amount must strictly be equal to "
                    "the balance.")
                )
    ...
    def _check_tax_lock_date(self, date=None):
        for line in self:
            move = line.move_id
            date = date or move.date
            if move.company_id.tax_lock_date and date <= move.company_id.tax_lock_date and line._affect_tax_report():
    ...
    def new(self, values={}, origin=None, ref=None):
        # OVERRIDE
        line = super().new(values=values, origin=origin, ref=ref)

        if line.move_id.is_invoice(include_receipts=True) and not line.exclude_from_invoice_tab:
            is_amount_currency_changed = 'amount_currency' in values
            is_balance_changed = 'debit' in values or 'credit' in values
    ...
    def create(self, vals_list):
        ...
        lines._check_amount_currency_equals_balance()
    ...
    def write(self, vals):
        ...
        if not self._context.get('write_recursion'):
            for line, cleaned_vals, snapshot0 in zip(self.with_context(write_recursion=True), vals_list, snapshots0):
                ...
                # Check hash.
                if move.restrict_mode_hash_table and move.state == 'posted' and snapshot0.any_has_changed(INTEGRITY_HASH_LINE_FIELDS):
                ...
                # Check the lock date.
                if snapshot0.any_has_changed(PROTECTED_FIELDS_LOCK_DATE):
                    line.move_id._check_fiscalyear_lock_date()

                # Check the tax lock date.
                if snapshot0.any_has_changed(PROTECTED_FIELDS_TAX_LOCK_DATE):
                    line._check_tax_lock_date()

                # Check the reconciliation.
                if snapshot0.any_has_changed(PROTECTED_FIELDS_RECONCILIATION):
                    line._check_reconciliation()

                if snapshot0.has_changed('account_id'):

                    # Check for deprecated account.
                    if line.account_id.deprecated:
                        raise UserError(_('You cannot use a deprecated account.'))

                    previous_account = snapshot0['account_id']
            ...
            self.move_id._synchronize_business_models()
            self._check_amount_currency_equals_balance()
    ...
    #     if self._context.get('line_ids') and any(field_name in default_fields for field_name in ('debit', 'credit', 'account_id', 'partner_id')):
