...
class Mailing(models.Model):
    ...
    def _prepare_statistics_email_values(self):
        ...
        if self.mailing_type == 'sms':
            ...
            values['title'] = _('24H Stats of %s "%s"') % (mailing_type, self.subject)
            values['kpi_data'][0] = {
                'kpi_fullname': _('Report for %i %s Sent') % (self._get_count_sent(), mailing_type),
                'kpi_col1': {
                ...
                'kpi_col2': {
                    ...
                    'col_subtitle': '%s (%i)' % (_('CLICKED'), self.clicked),
                },
                'kpi_col3': {
                    'value': f'{self.bounced_ratio}%',
                    'col_subtitle': '%s (%i)' % (_('BOUNCED'), self.bounced),
                },
                'kpi_action': None,
            }
        return values
    ...
    def _get_count_sent(self):
        if self.mailing_type == 'sms':
            return self.expected
        return super(Mailing, self)._get_count_sent()
